<?php
namespace Appseconnect\Twilio\Observer;
...
class SendOrderNotification implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        try {
            ...
            $client->messages->create($telephone, $params);

            $client2 = new \Twilio\Rest\Client($this->getGeneralConfig('account_sid'), $this->encryptor->decrypt($this->getGeneralConfig('auth_token')));
            $returntwilio = $client2->messages->create(
                "whatsapp:".$telephone,
                array('from' => "whatsapp:".$this->getGeneralConfig('send_from_number'),'body' => $this->getBody($order))
            );
            if($returntwilio->status == 'undelivered')
            {
                $this->logger->info('undelivered');
            }
    ...
    public function getGeneralConfig($value)
    {
        return $this->scopeConfig->getValue(
            "appseconnect_twilio/general/$value",
