...
class MassDelete extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($this->getRequest()->getParam('excluded') && $excludedStatus == 'false') {
            ...
            foreach ($multipleDiscountCollection as $multipleDiscount) {
                $this->unmap($multipleDiscount->getId());
            }
     ...
     * @return void
     */
    private function unmap($multipleDiscountId)
    {
        $this->multipleDiscountFactory->create()
            ->load($multipleDiscountId)
            ->delete();
    }
}