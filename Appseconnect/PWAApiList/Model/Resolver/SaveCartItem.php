...
use Appseconnect\B2BMage\Helper\CustomerTierPrice\Data as TierPriceHelper;
use Appseconnect\B2BMage\Helper\ContactPerson\Data as HelperData;
...
class SaveCartItem implements ResolverInterface
{
    ...
    public function resolve(
        ...
        if ($itemId) {
            ...
            $cartItem->setQty($qty);
            $this->updateItemPrice($context, null, $qty, $quote, $cartItem);
            ...
            $this->updateItemPrice($context, $product, $qty, $quote);
     ...
     * @param null $cartItem
     */
    public function updateItemPrice($context, $product, $qty, $quote, $cartItem = null)
    {
        ...
        $sku = $product->getSku();
        $finalPrize = $product->getFinalPrice($qty);
                ...
                foreach ($quote->getAllItems() as $item) {
                    if ($item->getSku() == $sku) {
                        ...
                        $customerSpecialPrice = $this->specialPriceHelper
                            ->getSpecialPrice($productId, $sku, $customerId, $websiteId, $productPrice);
                        //for category discount price
                        ...
                        if (!empty($tierData) || $customerSpecialPrice || $categoryDiscountedPrice || $pricelistPrice) {
                            $item->setCustomPrice($actualPrice);
                            if ($actualPrice <= 0) {
                                $item->setDiscountAmount($productPrice);
                                $item->setBaseDiscountAmount($productPrice);
                            }
